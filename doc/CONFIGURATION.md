...
| `group_file_path` | 否 | 空 | QQ 群文件上传路径，例如 `文档` 或 `文档/漫画`；留空时发送到群文件根目录。 |
...
`group_file_path` 只对 QQ 群聊的 `aiocqhttp` 适配器生效，私聊仍使用普通文件发送。插件会按名称逐级查询已经存在的群文件夹并使用对应的文件夹 ID 上传；如果路径不存在或机器人没有查询、上传群文件的权限，本次文件发送会失败并在 AstrBot 日志中记录原因，不会回退到根目录。
...
管理页面提供一次性登录功能。选择域名并输入 JM 账号、密码后，插件会在服务端登录并仅将返回的 Cookie 写入 `client.cookies`；账号和密码不会写入插件配置。建议通过 HTTPS 访问 AstrBot 管理页面，并避免在不受信任的共享设备上使用此功能。
...
健康域名按延迟从低到高排列，未检查域名保持配置顺序，不可用域名放到最后。检查结果会显示入口域名、真实响应域名和重定向次数。

管理页面的“自动发现”会综合当前配置、JMComic 永久入口、发布页和 GitHub 回退源生成候选列表。为避免一次检查大量新候选导致管理页面请求超时，发现操作不会同步批量检查所有候选；勾选并应用后，再点击“立即检查域名”确认连通性。章节检查和下载遇到“本子不存在”、请求重试耗尽或站点响应无法解析时，会继续尝试下一个候选域名。

自动发现只提供当前网络环境下的候选结果，不保证站点长期可用。域名健康检查仅检查首页连通性，无法判断 Cookie 是否有效或特定本子是否有权访问。若入口被重定向后登录态失效，或任务提示“疑似 Cookie 失效”，请对实际可用域名执行一次性登录以刷新 Cookie。
