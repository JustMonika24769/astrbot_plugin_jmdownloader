...
管理页面的“自动发现”会综合当前配置、JMComic 永久入口、发布页和 GitHub 回退源生成候选列表。确认健康状态后勾选域名并应用，即可更新 `client.domain`。章节检查和下载遇到“本子不存在”或请求重试耗尽时，还会继续尝试下一个候选域名。

自动发现只提供当前网络环境下的候选结果，不保证站点长期可用。若入口被重定向后登录态失效，请对实际可用域名执行一次性登录以刷新 Cookie。
